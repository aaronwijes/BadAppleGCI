...
def main():
    ...
    while True:
        ...
        img_pil = img_pil.resize((14, 14), Image.LANCZOS)
        img_pil = img_pil.convert('1', dither=Image.FLOYDSTEINBERG)

        # --- Write raw array data ---
        frame_temp = np.array(img_pil).astype(np.uint8).tolist()
        frame_1 = frame_temp[0:7]
        frame_2 = frame_temp[0:7]
        frame_3 = frame_temp[7:13]
        frame_4 = frame_temp[7:13]
